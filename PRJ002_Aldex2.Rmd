...
load relavent data
```{r}
load('sig.features.abs1.aldex.rds') # contains 1 phyloseq object and all stats for each tp
load('sig.features.abs2.aldex.rds') # contains 1 phyloseq object and all stats for each tp
```
...
write.csv(taxa,'maggie_plots/DA-taxa.csv')
```
...
ggsave('maggie_plots/Figure3-withleg.pdf',width=10,height=11)
```
