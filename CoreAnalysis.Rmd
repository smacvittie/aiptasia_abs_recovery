...
fb2<-fb + guides(color=F, fill=F, shape=F)

a + fb2 + c + d + e2 + plot_layout(design=layout3)

ggsave('maggie_plots/Figure2-new.pdf', width=11,height=10)
```
...
head(df.taxa)
write.csv(df.taxa, 'maggie_plots/core.taxa.csv')
```
...
ggsave('maggie_plots/figs1.pdf',height=6,width=12)
```
